...
for(round in 1:4){
 for(region in regions){
    ...
    region.subset %>% arrange(Order) -> region.subset
    ...
    while(i < nrow(region.subset)){
      ...
      if(p > (1-p)){
        region.subset[i,]$p = p
        region.subset[i,]$alpha.probability = compare_teams(region.subset[i,]$TeamID,
                region.subset[i+1,]$TeamID)[["q"]] %>% as.vector() > 0
        matchups = rbind(matchups, region.subset[i,])
      } else {
        region.subset[i+1,]$p = 1-p
        region.subset[i+1,]$alpha.probability = compare_teams(region.subset[i+1,]$TeamID,
